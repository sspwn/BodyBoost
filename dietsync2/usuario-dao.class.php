...
class UsuarioModel
{
    ...
    public function CadastrarUser($nome, $sobrenome, $tipo_user, $meta, $sexo, $data_nasc, $peso, $altura, $email, $senha)
    {
        $comando = $this->pdo->prepare("INSERT INTO users(name, sobrenome, tipo_user, meta, sexo, data_nasc, peso, altura, email, password) VALUES(:name,:sobrenome, :tipo_user, :meta, :sexo, :data, :peso, :altura, :email, :password)");

        $comando->bindValue(":name", $nome);
        $comando->bindValue(":sobrenome", $sobrenome);
        $comando->bindValue(":tipo_user", $tipo_user);
